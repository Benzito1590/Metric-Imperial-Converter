...
suite('Unit Tests', function () {
  ...
  suite('Function convertHandler.getNum(input)', function () {
    ...
    test('Decimal Input', function (done) {
      var input = '32.283L';
      assert.equal(convertHandler.getNum(input), 32.283);
      done();
    });
    ...
    test('Fractional Input w/ Decimal', function (done) {
      var input = '3.5/4.2L';
      assert.equal(convertHandler.getNum(input), 0.83333);
      done();
    });

    test('Invalid Input (double fraction)', function (done) {
      var input = '3.5/4.2/2.5L';
  ...
  suite('Function convertHandler.getUnit(input)', function () {
    ...
    test('For Each Valid Unit Inputs', function (done) {
      var units = Object.keys(metricImperial.validUnits);
    ...
    test('Unknown Unit Input', function (done) {
      var units = Object.keys(metricImperial.validUnits);
      assert.equal(convertHandler.getUnit('unknown'), false);
      done();
    });
